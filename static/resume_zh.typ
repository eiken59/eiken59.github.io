...
#section-block(
  ...
  [ 
    *一維與二維簡單隨機漫步之擊中時間研究* #h(1fr) 2025 年 7 月 -- 2026 年 2 月
    ...
    #v(0.4em)
    *一維隨機漫步的賭徒破產問題：簡單對稱隨機漫步與延伸* #h(1fr) 2025 年 9 月 -- 2025 年 12 月

    國立陽明交通大學應用數學系專題研究（一） #h(1fr) 指導教授：千野由喜老師

    - 研究賭徒在有限資金下的破產問題與獲勝機率。在簡單對稱隨機漫步中，獲勝機率具明確的解析解（即初始資金與目標資金之比）。本專題進一步探討「有限步長」及「有限變異數」的非簡單對稱情形；即使在無解析解的情況下，其獲勝機率仍與該資金比例存在預期的線性界限（linear scaling）關係。
    ...
    *#link("https://github.com/eiken59/ml_project")[基於機器學習之信用卡違約偵測]* #h(1fr) 2025 年 2 月 -- 2025 年 6 月
    ...
    - 使用 Python (`scikit-learn`) 處理此信用卡違約真實數據，訓練並評估五種機器學習模型。針對資料不平衡問題，採用 F1-score 作為核心評估指標；研究發現，具備高解釋性的決策樹與羅吉斯迴歸模型，在此分類任務的預測表現與複雜的 XGBoost 相當，展現了兼顧模型解釋性與準確度之能力。
    ...
    *#link("https://hackmd.io/@eiken-sc11/B1QcaW3B0")[進入金融前的數學：隨機漫步與平賭]* #h(1fr) 2024 年 6 月 -- 2024 年 7 月
    ...
    - 探討賭徒破產問題在一維簡單隨機遊走的遊戲時長以及賭徒獲勝機率。此外，一維簡單隨機遊走中從 0 至 1 的擊中時間分布也被探討。在隨機遊走以外，我們討論了測度論底下平賭的一些性質，包含可選抽樣定理（optional sampling theorem）。
  ],
)
